...
export default function GameToastPopup() {
  ...
  useEffect(() => {
    ...
    async function fetchPopup() {
      try {
        ...
        if (data?.success && data?.config && (data.config.enabled === true || data.config.enabled === 'true')) {
      ...
      channel = supabase
        ...
        .on('broadcast', { event: 'GAME_POPUP_UPDATED' }, (payload: any) => {
          if (payload?.payload) {
            ...
            if (newConfig?.enabled === true || newConfig?.enabled === 'true') {
              setConfig(newConfig);
              setVisible(true);
              setClosing(false);
            } else {
    ...
    const handleGlobalSync = async () => {
      try {
        ...
        if (data?.success && data?.config) {
          if (data.config.enabled === true || data.config.enabled === 'true') {
            setConfig(data.config);
            setVisible(true);
            setClosing(false);
          } else {
  ...
  if (!visible || !config || (config.enabled !== true && (config.enabled as any) !== 'true')) {
    return null;
  }

  const iconStr = String(config.icon || '🎮').trim();
  const isImg =
    iconStr.startsWith('http://') ||
    iconStr.startsWith('https://') ||
    iconStr.startsWith('/');

  const titleStr = config.title || 'TANGAZO LA GAME';
  const messageStr = config.message || 'Cheza game mpya kwa kubonyeza hapa chini!';
  const btnTextStr = config.btn_text || 'FUNGUA HAPA';
  const btnLinkStr = config.btn_link || '#';
  const badgeStr = config.badge || '';
